...
def _load_page_object(page_json: Mapping, connection: sqlite3.Connection):
    ...
    # Some endpoints will return responses without data (for example if all of
    # the tweets in a hydration call are no longer available)
    for tweet in page_json.get("data", []):
